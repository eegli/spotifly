...
test('Tracks', async () => {
  {
    const res = await Spotifly.Tracks.Track.get(trackId);
    expect(res.data.id).toEqual(trackId);
  }
  {
    const res = await Spotifly.Tracks.Track.getSeveral(trackIds50);
    expect(res.data.tracks.map(t => t.id)).toEqual(trackIds50);
  }
  {
    const res = await Spotifly.Tracks.Track.getAll(trackIds51)();
