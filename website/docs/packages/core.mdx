...
Examples are `Tracks.getAllUsersSavedTracks` (paginated) `Tracks.getAllTracks` (limited).

The Spotify Web API returns at most 50 items when asked for a user's saved tracks and allows no more than 50 track ids when querying for track catalog information.
...
The following example returns all of a user's saved tracks in a flat array. Convenience methods have a different signature: **They are curried and return a function which accepts an optional callback**. Each time a new chunk of items is fetched, the response of type `DataResponse` is passed to the callback as the only argument.

Even asynchronous callbacks are allowed! Their return values are discharged.
...
Convenience methods simply wrap their ordinary counterpart, which is reflected in their return types. The convenience method returns an array of the type that the basic method returns:
