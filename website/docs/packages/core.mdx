...
The type `SpotifyClient` mirrors the structure of an _initialized_ client. `DataCallback` can be used to infer the signature of the callback to a curried function (convenience method). Both types can be useful in some scenarios:
